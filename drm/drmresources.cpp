...
int DrmResources::UpdatePrimary() {
...
bool DrmResources::mode_verify(const DrmMode &m) {
  if (!white_modes_.size())
    return true;
  printf("white_modes_.size=%d\n", white_modes_.size());
  for (const DrmMode &mode : white_modes_) {
    
    //printf(">>>>>>mode_verify:%d,%d,%f,%d, %d,%d ,%d ,%d\n",\
    //       m.h_display(), m.v_display(), m.v_refresh(), m.flags(), m.h_sync_start(),\
    //       m.h_sync_end(),m.h_total(),m.clock());

    //printf("<<<<<<mode_verify:%d,%d,%f,%d, %d,%d ,%d ,%d\n",\
    //       mode.h_display(), mode.v_display(), mode.v_refresh(), mode.flags(), mode.h_sync_start(),\
    //       mode.h_sync_end(),mode.h_total(),mode.clock());

    if(mode.h_display() == m.h_display() && mode.v_display() == m.v_display() && mode.v_refresh() == m.v_refresh() && mode.flags() == m.flags()
        && mode.h_sync_start() == m.h_sync_start() && mode.h_sync_end() == m.h_sync_end()
        && mode.h_total() == m.h_total() && mode.clock()==m.clock()) {
...
void DrmResources::ConfigurePossibleDisplays()
{
  ...
  strcpy(primary_name, "HDMI-A,TV");
  strcpy(extend_name, "hdmi");
  
  primary_length = 1;//property_get("sys.hwc.device.primary", primary_name, NULL);
  extend_length = 0;//property_get("sys.hwc.device.extend", extend_name, NULL);
...
int DrmResources::Init(int drm_fd) {
	
  char path[PROPERTY_VALUE_MAX];
 // property_get("hwc.drm.device", path, "/dev/dri/card0");
  memset(path, 0, sizeof(path));
  strcpy(path, "/dev/dri/card0");
  init_white_modes();
  /* TODO: Use drmOpenControl here instead */
  _drm_fd = drm_fd;//open(path, O_RDWR);
//  fd_.Set();
  
  fd_.Set(_drm_fd);
  if (fd() < 0) {
    printf("Failed to open dri- %s", strerror(-errno));
    return -ENODEV;
  }
 printf("DrmResources::drmSetClientCap,fd=%d,%p\n",fd(),this);
  int ret = drmSetClientCap(fd(), DRM_CLIENT_CAP_UNIVERSAL_PLANES, 1);
  if (ret) {
    printf("Failed to set universal plane cap %d", ret);
    return ret;
  }
...
#endif
   printf("DrmResources::drmModeGetResources\n");
  ...
  out << "CRTCs:\n";
  out << "id\tfb\tpos\tsize\n";
  printf("DrmResources::drmModeGetCrtc\n");
  ...
  out << "id\tcrtc\ttype\tpossible crtcs\tpossible clones\t\n";
   printf("DrmResources::drmModeGetEncoder encodecount=%d\n",res->count_encoders);
  ...
  out.str("");
   ...
   printf("DrmResources::Connectors,%d\n",res->count_connectors);
  out << "Connectors:\n";
...
//    printf("++++++found_primary=%d\n",found_primary);
  ...
  if (!found_primary) {
    for (auto &conn : connectors_) {
	    printf("possible_displays=%d,built_in=%d,state=%d,%d\n",conn->possible_displays(),conn->built_in(),conn->state(),DRM_MODE_CONNECTED);
      if (!(conn->possible_displays() & HWC_DISPLAY_PRIMARY_BIT))
        continue;
      found_primary = true;
      SetPrimaryDisplay(conn.get());
  ...
  std::sort(sort_planes_.begin(),sort_planes_.end(),PlaneSortByZpos);
    ...
    for (std::vector<DrmPlane*>::const_iterator iter= sort_planes_.begin();
       ...
       printf("sort_planes_ share_id=%" PRIu64 ",zpos=%" PRIu64 "",share_id,zpos);
    }
...
int  DrmResources::commitFrame(int fb_id, int crtc_id) {
   ...
   for (std::vector<DrmPlane*>::const_iterator iter= sort_planes_.begin();
	...
	ret = drmModeAtomicAddProperty(pset, plane->id(),
                                   plane->crtc_property().id(), crtc_id) < 0;
    ret |= drmModeAtomicAddProperty(pset, plane->id(),
    ...
    uint32_t flags = 0;
    ret = drmModeAtomicCommit(_drm_fd, pset, flags, this);
    if (ret < 0) {
...
{
  int timeline = 0;//property_get_int32("sys.display.timeline", -1);
  int ret;
  /*
...
int DrmResources::UpdateDisplaySize(int src_xpos, int src_ypos, int src_w, int src_h, int dst_xpos, int dst_ypos, int dst_w, int dst_h) {

   printf("###########UpdateDisplaySize:fd=%d,%p,n",_drm_fd,this);
   drmModeAtomicReqPtr pset = drmModeAtomicAlloc();
    if (!pset) {
        ALOGE("Failed to allocate property set");
        return -ENOMEM;
   ...
   for (std::vector<DrmPlane*>::const_iterator iter= sort_planes_.begin();
    ...
    uint32_t flags = 0;
    ret = drmModeAtomicCommit(_drm_fd, pset, flags, this);
    printf("+++++++======UpdateDisplaySize drmModeAtomicCommit,ret=%d,%s,fd=%d\n",ret,strerror(-errno),_drm_fd);
    if (ret < 0) {
        printf("Failed to commit pset ret=%d\n", ret);
        drmModeAtomicFree(pset);
        return ret;
...
int DrmResources::UpdateDisplayRoute(void)
{
  ...
  DrmConnector *extend = NULL;//GetConnectorFromType(HWC_DISPLAY_EXTERNAL);
  if (enable_changed_) {
 ...
 */
  ...
  for (i = 0; i < HWC_NUM_PHYSICAL_DISPLAY_TYPES; i++) {
    ...
    if (!conn)
      continue;
    printf("state=%d,id=%d,encoder=%d,crtc=%d\n",conn->state(),conn->current_mode().id(),conn->encoder(),conn->encoder()->crtc());
    if (conn->state() != DRM_MODE_CONNECTED ||
        !conn->current_mode().id() || !conn->encoder() ||
        !conn->encoder()->crtc()) {
	...
	printf("ADD_PROP mode_property().id\n");
  }
  /*
...
int DrmResources::CreatePropertyBlob(void *data, size_t length,
  ...
  int ret = drmIoctl(_drm_fd, DRM_IOCTL_MODE_CREATEPROPBLOB, &create_blob);
  if (ret) {
    return ret;
  }
...
int DrmResources::DestroyPropertyBlob(uint32_t blob_id) {
  ...
  int ret = drmIoctl(_drm_fd, DRM_IOCTL_MODE_DESTROYPROPBLOB, &destroy_blob);
  if (ret) {
...
int DrmResources::GetProperty(uint32_t obj_id, uint32_t obj_type,
  ...
  props = drmModeObjectGetProperties(_drm_fd, obj_id, obj_type);
  if (!props) {
  ...
  for (int i = 0; !found && (size_t)i < props->count_props; ++i) {
    drmModePropertyPtr p = drmModeGetProperty(_drm_fd, props->props[i]);
...
void DrmResources::dump_blob(uint32_t blob_id, std::ostringstream *out) {
	...
	blob = drmModeGetPropertyBlob(_drm_fd, blob_id);
	if (!blob) {
		*out << "\n";
		return;
	}
...
bool DrmResources::is_hdr_panel_support_st2084(DrmConnector *conn) const {
	...
	blob = drmModeGetPropertyBlob(_drm_fd, hdr_panel_prop->blob_ids[0]);
	if (!blob) {
