                    ...
                    uint32_t flags) const
{
  ...
  if (fabs(v_refresh - vrefresh) > 1.0f)
    return false;
  printf("h_display_=%d,%d,v_display_=%d,%d,hsync_start=%d,%d,hsync_end=%d,%d,vsync_start=%d,%d,vsync_end=%d,%d,htotal=%d,%d,vtotal=%d,%d,flags=%d,%d\n",\
         h_display_ , width,v_display_ , height,hsync_start , h_sync_start_,vsync_end , v_sync_end_,htotal , h_total_,vtotal , v_total_,flags , flags_);
  if (h_display_ == width && v_display_ == height &&
